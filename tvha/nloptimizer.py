...
class NLoptOptimizer(Optimizer):
    ...
    def __init__(self, max_evals: int = 1000) -> None:  # pylint: disable=unused-argument
        ...
        self._optimizer_names = {
            ...
            NLoptOptimizerType.LN_SBPLX: nlopt.LN_SBPLX,  # added by poc
        }
    ...
    def minimize(
        ...
        if bounds is None:
            bounds = [(None, None)] * x0.size  # type: ignore

        threshold = 3 * np.pi
        low = [(lower if lower is not None else -threshold) for (lower, upper) in bounds]
        high = [(upper if upper is not None else threshold) for (lower, upper) in bounds]
        ...
        def wrap_objfunc_global(x, _grad):  # type: ignore
            nonlocal eval_count
            eval_count += 1
            return fun(x)
